...
void OLED_Refresh(void)
{
	...
	for(i=0;i<8;i++)
	{
	   ...
	   OLED_WR_Byte((uint8)(0xb0+i),OLED_CMD); //设置行起始地址
...
//len :数字的位数
//size:字体大小
void OLED_ShowNum(uint8 x,uint8 y,u32 num,uint8 len,uint8 size1)
{
	uint8 t,temp;
	for(t=0;t<len;t++)
	{
		temp=(num/OLED_Pow(10,(uint8)(len-t-1)))%10;
			if(temp==0)
			{
				OLED_ShowChar((uint8)(x+(size1/2)*t),y,'0',size1);
      }
			else 
			{
			  OLED_ShowChar((uint8)(x+(size1/2)*t),y,(uint8)(temp+'0'),size1);
			}
  }
}
...
{
	OLED_WR_Byte((uint8)(0xb0+y),OLED_CMD);//设置行起始地址
...
void OLED_Init(void)
{
	...
	spi_init(OLED_SPI_N, OLED_SPI_SCLK_PIN, OLED_SPI_MOSI_PIN, OLED_SPI_MISO_PIN, 3, MASTER, SPI_SYSclk_DIV_4);
