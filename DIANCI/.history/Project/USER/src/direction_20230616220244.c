 ...
 * @LastEditTime: 2023-06-16 22:01:15
...
void Lose_Line_deal_yuanshu()
{
			if(L_2<=Lose_Line_Thres_chalu)//左丢线
				{
					if(R_2>L_2)
					{
						pwm_duty(PWMB_CH1_P74,servo_PWM_MIN);	//PWMA设置占空比
					}
				}
				else if(R_2<=Lose_Line_Thres_chalu)//右丢线
				{
					if(L_2>R_2)
					{
						pwm_duty(PWMB_CH1_P74,servo_PWM_MAX);	//PWMA设置占空比
					}
				}
//				else
//				{
