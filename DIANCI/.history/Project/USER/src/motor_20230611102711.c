 ...
 * @LastEditTime: 2023-06-11 10:26:11
 ...
 *  备    注：
 *************************************************************************/
//因为占空比过大会导致电机损坏，故我们取了70%为最大占空比（35000），由于1024线编码器用了四倍频最大maicho
...
//电机占空比限制
	if(out_duty_L >motorduty_Max){out_duty_L = motorduty_Max;}
	if(out_duty_L <motorduty_Min){out_duty_L = motorduty_Min;}	
    if(out_duty_R >motorduty_Min){out_duty_R = motorduty_Max;}
    if(out_duty_R <motorduty_Min){out_duty_R = motorduty_Min;}
